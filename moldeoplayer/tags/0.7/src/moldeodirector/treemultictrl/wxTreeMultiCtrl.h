...
#define WXTMC_GUTTER_DEFAULT 0      // gutter before and after image of [+] or [-]
...
private:
    ...
    /**
     FABRICIO COSTA: we really needed this one: find a node looking into node name
    */
    TreeMultiItemBase *FindNodeByName( TreeMultiItemBase *b, const wxTreeMultiItem& item );
...
public:
    ...
    */
    ...
    /** Adds a root node to the wxTreeMultiItem. There can be many root nodes. Use this wxTreeMultiNode pointer to add
        more subnodes to it.
        FABRICIO COSTA: Now you can add a window, just after the caption
    */
    wxTreeMultiItem AddRoot(const wxString &caption, const wxString &name = wxEmptyString, wxWindow* window = 0);
    ...
    wxTreeMultiItem AppendNode(const wxTreeMultiItem &item, const wxString &caption = wxEmptyString,
                            const wxString &name = wxEmptyString, wxWindow *window = 0 );
    ...
    */
    ...
    ///FABRICIO COSTA: Made to be called from an another control, based on the same item NAME!!!!
    void FoldTo(const wxTreeMultiItem &item, bool expand = true) {

        // scan all nodes to see which one matches
        TreeMultiItemBase *b = 0;
        for(int i = 0; i < _root.GetNodeCount() && !b; i++)
            b = FindNodeByName(_root.GetNode(i), item );
        ...
        // return an item
        wxTreeMultiItem item2 = wxTreeMultiItem(b);


        if(expand)
            Expand(item2, false);
        else
            Collapse(item2, false);
    };
    ...
    */
    ...
    /**
    * FABRICIO COSTA: Added to really show scrollbars or not whenever we needed it
    */
    void ShowScrollBars( bool horizontal = true, bool vertical = true ) {
            m_bShowHorizontal = horizontal;
            m_bShowVertical = vertical;
    }

    /**
    * FABRICIO COSTA: This control will control some aspect of a second one, related to it
    */
    void SetSecondTargetWindow( wxTreeMultiCtrl*  window ) {
        m_pSecondTargetWindow = window;
    }

    /**
    * FABRICIO COSTA: As it is related to a second window, OnScroll will scroll both...
    */
    void OnScroll( wxScrollWinEvent& event );

    /**
    *    FABRICIO COSTA: Whenever scrollbars aren't viewable, Scroll functions apperas to doesn't work
    *    at all, so we hace to scroll manually with ScrollWindow... this one keep track of the position
    *    as the normal Scroll
    */
    void ScrollWindowToPos( int x, int y );
...
private:
    ...
    wxTreeMultiCtrl* m_pSecondTargetWindow;
    int m_dy_scrolled;
    int m_dx_scrolled;
